...
class StorageManager {
  constructor(compatibilityLayer = null) {
    ...
    this.compatibilityLayer =
      compatibilityLayer || self.compatibilityLayer || chrome;
  ...
  async testStorage() {
    try {
      ...
      if (
        !this.compatibilityLayer ||
        !this.compatibilityLayer.storage ||
        !this.compatibilityLayer.storage.local
      ) {
  ...
  async saveTranslation(translation) {
    try {
      ...
      const maxHistory =
        settings.maxHistoryItems || APP_CONSTANTS.MAX_TRANSLATION_HISTORY;
  ...
  async getFavoriteTranslations() {
    try {
      ...
      const preferences =
        result[APP_CONSTANTS.STORAGE_KEYS.USER_PREFERENCES] || {};
  ...
  async getStatistics() {
    try {
      ...
      return (
        result[APP_CONSTANTS.STORAGE_KEYS.STATISTICS] || {
          totalTranslations: 0,
          totalCharacters: 0,
          languagesUsed: {},
          dailyStats: {},
  ...
  async updateStatistics(translation) {
    try {
      ...
      stats.totalCharacters =
        (stats.totalCharacters || 0) + translation.originalText.length;

      // Dil kullanım istatistikleri
      const targetLang = translation.targetLanguage.code;
      stats.languagesUsed[targetLang] =
        (stats.languagesUsed[targetLang] || 0) + 1;
      ...
      stats.dailyStats[today].languages[targetLang] =
        (stats.dailyStats[today].languages[targetLang] || 0) + 1;
      ...
      stats.weeklyStats[weekKey].languages[targetLang] =
        (stats.weeklyStats[weekKey].languages[targetLang] || 0) + 1;
      ...
      stats.monthlyStats[monthKey].characters +=
        translation.originalText.length;
      stats.monthlyStats[monthKey].languages[targetLang] =
        (stats.monthlyStats[monthKey].languages[targetLang] || 0) + 1;
  ...
  async getCacheData() {
    try {
      ...
      return (
        result[APP_CONSTANTS.STORAGE_KEYS.CACHE] || {
          translations: {},
          languageDetections: {},
  ...
  async getStorageUsage() {
    try {
      if (this.compatibilityLayer.storage.local.getBytesInUse) {
        const bytesInUse =
          await this.compatibilityLayer.storage.local.getBytesInUse();
        return {
          bytesInUse: bytesInUse,
          quota: this.compatibilityLayer.storage.local.QUOTA_BYTES || 5242880, // 5MB
          usagePercent:
            (bytesInUse /
              (this.compatibilityLayer.storage.local.QUOTA_BYTES || 5242880)) *
            100,
        };
      }
