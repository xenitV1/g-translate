    ...
    async detectLanguage(text) {
        try {
            ...
            const messages = [
                {
                    role: 'system',
                    content: 'Sen bir dil tespit uzmanısın. Verilen metnin hangi dilde yazıldığını tespit et ve sadece dil kodunu döndür (örn: tr, en, es, fr, de, it, pt, ru, ja, ko, zh, ar).'
                },
                {
                    role: 'user',
    ...
    async callOpenAIAPI(messages) {
        ...
        for (let attempt = 0; attempt < this.maxRetries; attempt++) {
            try {
                ...
                lastError = error;
                console.warn(`OpenAI API çağrısı başarısız (deneme ${attempt + 1}/${this.maxRetries}):`, error);
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = { OpenAIAPIHandler };
}

if (typeof window !== 'undefined') {
    window.OpenAIAPIHandler = OpenAIAPIHandler;
}
