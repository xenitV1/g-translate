...
class HistoryPopupController {
  ...
  attachEventListeners() {
    // Header buttons
    this.elements.clearHistoryBtn.addEventListener(
      "click",
      this.clearHistory.bind(this),
    );
    this.elements.closeHistoryBtn.addEventListener(
      "click",
      this.closeHistory.bind(this),
    );
    ...
    this.elements.languageFilter.addEventListener(
      "change",
      this.handleFilter.bind(this),
    );

    // Footer buttons
    this.elements.exportHistoryBtn.addEventListener(
      "click",
      this.exportHistory.bind(this),
    );
  ...
  async loadSettings() {
    try {
      ...
      const result = await compatibilityLayer.getStorageData([
  ...
  createHistoryItem(item) {
    ...
    // Metinleri güncelle
    const originalText = historyItem.querySelector(
      ".original-text .text-content",
    );
    const translatedText = historyItem.querySelector(
  ...
  async clearHistory() {
    try {
      if (
        confirm(
          "Tüm çeviri geçmişini silmek istediğinizden emin misiniz? Bu işlem geri alınamaz.",
        )
      ) {
        this.history = [];
